...
TEST_DATASET = part_pheno4d_single.PartNormalDataset(FILENAME)
...
def extract_output(points, labels, index, batch_idx):
  filepath = TEST_DATASET.datapath[index+BATCH_SIZE*batch_idx][1] # original txt
  base_filename = os.path.basename(filepath)
  output_folder = os.path.join(ROOT_DIR, 'data', 'single')
  if not os.path.exists(output_folder): os.makedirs(output_folder)
  filename, file_extension = os.path.splitext(base_filename)
  save_file = os.path.join(output_folder, filename + '_segmented.txt')
  print(save_file)
  ...
  with open(filepath, 'r') as fid:
    ...
    for line in fid:
      if line == '\n': continue
      nums = line.split()
      nums[-1] = "{0:.6f}".format(float(labels[count]))
