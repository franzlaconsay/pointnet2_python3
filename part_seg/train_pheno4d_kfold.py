...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(int(num_batches)):
        ...
        for i in range(cur_batch_size):
            ...
            matrix = confusion_matrix(segl, segp, labels = [0, 1])
            TP = matrix[1][1]
            TN = matrix[0][0]
            FP = matrix[0][1]
            FN = matrix[1][0]
...
if __name__ == "__main__":
    ...
    train_model = 'pointnet2_part_seg'
    ...
    for cat in categories:
        ...
        for ratio_i in range(len(ratios)):
            ...
            for k_i in enumerate(kfold.split(models)):
                ...
                MODEL = importlib.import_module(train_model) # import network module
                MODEL_FILE = os.path.join(ROOT_DIR, 'models', train_model+'.py')
                ...
                TRAIN_DATASET = Pheno4dNormalDataset(category=CATEGORY, indices=train_indices, train_samples=TRAIN_SAMPLE)
                TEST_DATASET = Pheno4dNormalDataset(category=CATEGORY, indices=test_indices)
